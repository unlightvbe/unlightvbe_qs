...
Attribute 通知表單是否已出現.VB_VarUserMemId = 1073741837
...
Sub 清除戰鬥系統所有變數值()
    ...
    Erase 戰鬥系統類.goicheck   '攻擊/防禦模式加骰數值檢查碼
    Erase 戰鬥系統類.liveus
    Erase 戰鬥系統類.livecom
    Erase 戰鬥系統類.liveusmax
    Erase 戰鬥系統類.livecommax
    Erase 戰鬥系統類.pageusleadmax  '使用者牌順序計數表(0.手牌/1.出牌)
    Erase 戰鬥系統類.pagecomleadmax   '電腦牌順序計數表(0.手牌/1.出牌)
    Erase 戰鬥系統類.pageqlead   '出牌計數變數(1.使用者/2.電腦)
    Erase 戰鬥系統類.pageglead   '手牌計數變數(1.使用者/2.電腦)
    戰鬥系統類.movedsus = 0   '使用者移動階段決定值變數
    戰鬥系統類.turnpageonin = 0    '階段是否可出牌變數(一般)
    戰鬥系統類.turnpageoninatking = 0    '階段是否可出牌變數(技能使用)
    戰鬥系統類.goickus = 0    '牌值一次檢查碼
    Erase 戰鬥系統類.atkingck   '技能階段啟動碼(x.人物技能編號,1.技能執行階段/2.技能啟動檢查值)
    Erase 戰鬥系統類.atkingtrn  '技能計數器暫時儲存變數(1.使用者(現)/2.電腦(現)/3.使用者(備份)/4.電腦(備份))
    戰鬥系統類.HP檢查變數 = False    'HP檢查階段是否已檢查變數
    戰鬥系統類.HP檢查階段數 = 0  'HP檢查階段變數(1.移動階段後,2.攻擊/防禦階段前,3.攻/防禦階段後)
    Erase 戰鬥系統類.距離單位  '距離單位暫時儲存資料(1.HP血條/2.牌移動,1.使用者/2.電腦,1.Left單位/2.Top單位)
    Erase 戰鬥系統類.personminixy    '小人物圖片座標指定資料(1.使用者/2.電腦,第n位,1.近距離/2.中距離/3.遠距離,1.Left/2.Top(座標))
    Erase 戰鬥系統類.異常狀態檢查數    '異常狀態啟動碼(x.異常狀態編號,1.狀態執行階段/2.狀態啟動檢查值)
    戰鬥系統類.技能動畫顯示階段數 = 0    '技能動畫計數器階段碼(1.攻擊/防禦階段-普通,2.移動階段-普通/3.發牌階段後、移動階段前/4.移動階段後/5.攻擊階段後/6.防禦階段後/7.回合結束時)
    Erase 戰鬥系統類.攻擊防禦骰子總數    '攻擊/防禦模式骰子數量資料(1.使用者(總)/2.電腦(總)/3.使用者(原)/4.電腦(原))
    Erase 戰鬥系統類.atkingpagetot  '每階段出牌種類及數值統計資料(1.使用者/2.電腦,1.劍/2.防/3.移/4.特/5.槍)
    Erase 戰鬥系統類.骰數零檢查值  '當前階段骰子數量是否為零檢查數(1.使用者/2.電腦)
    Erase 戰鬥系統類.牌總階段數    '牌擁有總階段數(1.使用者/2.電腦/3.總計)
    Erase 戰鬥系統類.牌移動暫時變數    '牌移動計數器暫時變數(1.Left單位/2.Top單位/3.牌張編號)
    Erase 戰鬥系統類.目前數  '總暫時變數
    Erase 戰鬥系統類.距離單位_收牌暫時數  '收牌個別距離單位暫時儲存變數(第x順序,1.Left單位/2.Top單位/3.牌張編號)
    戰鬥系統類.階段狀態數 = 0    '每階段開始結束狀態檢查數(1.開始階段/2.結束階段)
    Erase 戰鬥系統類.小人物頭像移動方向數   '小人物頭像移動方向狀態數(1.使用者/2.電腦[1.向內,2.向外])
    Erase 戰鬥系統類.血量計數器動畫暫時變數    '開始初始階段-血量動畫計數器暫時變數(1.使用者血條/2.電腦血條,1.每次移動量/2.是否已完成)
    Erase 戰鬥系統類.時間軸顏色變化紀錄暫時變數    '時間軸進行顏色變化階段紀錄暫時變數(1~3(1)單位變化量(1(1).時間軸(內外))/2.目前累計量/3.目前顏色(R,G,B),4.(1)時間軸(外)階段數-(1)黑變紅-(2)紅變黑/2.目前累計量/3.目前顏色(R))
    Erase 戰鬥系統類.開始卡片移動動畫完成數   '開始時每張卡片移動動畫完成紀錄數(1.使用者/2.電腦,1~3.卡片/4.目前第幾張)
    Erase 戰鬥系統類.交換角色紀錄暫時變數    '交換角色雙方紀錄暫時數(1.使用者/2.電腦/3.是否當下首次/4.交換角色完執行階段數)
    戰鬥系統類.戰鬥模式勝敗紀錄數 = 0    '戰鬥系統當前勝敗紀錄暫時變數(1.使用者方勝利/2.使用者方敗北/3.平手)
    戰鬥系統類.電腦方移動階段選擇數 = 0
    戰鬥系統類.電腦方事件卡是否出完選擇數 = False
    戰鬥系統類.是否系統公骰 = False
    Erase 戰鬥系統類.人物卡面背面編號紀錄數  '人物卡片背面技能說明人物編號暫時變數(1.(1).使用者/(2).電腦,2.第n位)
    Erase 戰鬥系統類.擲骰表單溝通暫時變數    'Form6表單值溝通暫時變數(1.一回合中先後判斷(1.前/2.後),2.原始骰值(使用者)-擲骰後有效傷害數,3.原始骰值(電腦)-擲骰後傷害對象(1.使用者/2.電腦),4.(1.使用者先攻/2.電腦先攻))
    Erase 戰鬥系統類.公用牌各牌類型紀錄數    '各場景公用牌牌類型紀錄暫時變數(0.(1)目前已發牌總數量/(2)目前場景牌總數量,1~31.(1)目前已使用之牌數/(2)該牌型能使用之總數量)
    Erase 戰鬥系統類.戰鬥擲骰介面人物立繪圖路徑紀錄數    '戰鬥系統擲骰介面雙方人物立繪圖路徑紀錄數(1.使用者方/2.電腦方)
    Erase 戰鬥系統類.人物實際狀態資料庫    '人物實際狀態資料
    戰鬥系統類.ActionCardTotNum = 0    '戰鬥系統卡牌總發行紀錄數
    '===================
    Erase 事件卡記錄暫時數    '事件卡使用紀錄暫時變數(0.(1)總共給予回合數,1.使用者/2.電腦,1.總共數值/2.目前處理數值/3.目前階段/4.事件卡牌編號/5.事件分類/6.是否啟動)
    ''===================
    執行指令集.vbecommadtotplay = 0
    執行階段系統類.VBEPersonVSTempStageNum(1) = -999
    執行階段系統類.VBEPersonVSTempStageNum(2) = 0
    ReDim 執行指令集.vbecommadnum(1 To 7, 執行指令集.vbecommadtotplay)
    ReDim 執行指令集.vbecommadstr(1 To 3, 執行指令集.vbecommadtotplay)
    Erase 執行指令集.Vss_PersonAtkingOffNum
    Erase 執行指令集.Vss_AtkingInformationRecordStr
    ReDim 執行階段系統類.VBEStageNum(0) As Integer
    For i = 1 To 2
        For j = 1 To UBound(戰鬥系統類.人物異常狀態列表, 2)
            Set 戰鬥系統類.人物異常狀態列表(i, j) = Nothing
        Next
    Next
    For i = 0 To UBound(戰鬥系統類.CardDeckCollection)
        Set 戰鬥系統類.CardDeckCollection(i) = Nothing
    Next
